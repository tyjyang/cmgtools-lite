#include "TRandom3.h"
#include "TMath.h"
#include "RoccoR80XMC.h"
//#include "__utils__/RoccoR80XMC.h"


int RocRes80XMC::getBin(double x, const int NN, const double *b){
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

RocRes80XMC::RocRes80XMC():NETA(1),NTRK(1),NMIN(1){
...
int RocRes80XMC::getEtaBin(double feta){
    return getBin(feta,NETA,BETA);
}

int RocRes80XMC::getNBinDT(double v, int H){
    return getBin(v,NTRK,dtrk[H]);
}

int RocRes80XMC::getNBinMC(double v, int H){
    return getBin(v,NTRK,ntrk[H]);
}

void RocRes80XMC::dumpParams(){
    cout << NMIN << endl;
    cout << NTRK << endl;
    cout << NETA << endl;
...
void RocRes80XMC::init(string filename){
...
double RocRes80XMC::Sigma(double pt, int H, int F){
    double dpt=pt-45;
    return rmsA[H][F] + rmsB[H][F]*dpt + rmsC[H][F]*dpt*dpt;
}

double RocRes80XMC::kSpreadDet(double gpt, double rpt, double eta, int nlayers, double w){
...
double RocRes80XMC::kSmearDet(double pt, double eta, TYPE type, double v, double u){
    int H = getBin(fabs(eta), NETA, BETA);
...
double RocRes80XMC::kExtraDet(double pt, double eta, int nlayers, double u, double w){
    int H = getBin(fabs(eta), NETA, BETA);
    int F = nlayers-NMIN;
    double  v = ntrk[H][F]+(ntrk[H][F+1]-ntrk[H][F])*w;
    int     D = getBin(v, NTRK, dtrk[H]);
...
void RocRes80XMC::fillFitData(int &H, int &F, int &D, double &xmc, double &xdt, double &Rmc, double &Rdt, double pt, double eta){
...
double RocOne::kGenSmearDet(double pt, double eta, double v, double u){
    return RR.kSmearDet(pt, eta, RocRes80XMC::Data, v, u);
}
...
const int RoccoR80XMC::Nmem[RoccoR80XMC::Nset]={1, 100, 1, 41, 21};

RoccoR80XMC::RoccoR80XMC(){
...
RoccoR80XMC::RoccoR80XMC(std::string dirname){
    for(int s=0; s<Nset; ++s){
	for(int m=0; m<Nmem[s]; ++m){
...
double RoccoR80XMC::kScaleDT(int Q, double pt, double eta, double phi, TYPE T, int m){
    return RC[T][m]->kScaleDT(Q, pt, eta, phi);
}


double RoccoR80XMC::kScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers){
    return RC[Default][0]->kScaleAndSmearMCDet(Q, pt, eta, phi, nlayers, random.Rndm(), random.Rndm());

}
	
double RoccoR80XMC::kScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers){
    return RC[Default][0]->kScaleFromGenMCDet(Q, pt, eta, phi, gpt, nlayers, random.Rndm());
}

double RoccoR80XMC::kScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers, double u, double w, TYPE T, int m){
    return RC[T][m]->kScaleAndSmearMCDet(Q, pt, eta, phi, nlayers, u, w);
}

double RoccoR80XMC::kScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers, double w, TYPE T, int m){
    return RC[T][m]->kScaleFromGenMCDet(Q, pt, eta, phi, gpt, nlayers, w);
}

std::vector<std::vector<double> > RoccoR80XMC::kkScaleAndSmearMCDet(int Q, double pt, double eta, double phi, int nlayers){
...
std::vector<std::vector<double> > RoccoR80XMC::kkScaleFromGenMCDet(int Q, double pt, double eta, double phi, double gpt, int nlayers){
    double u=random.Rndm();
    double w=random.Rndm();
