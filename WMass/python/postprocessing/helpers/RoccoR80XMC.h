#ifndef ElectroWeakAnalysis_RoccoR80XMC
#define ElectroWeakAnalysis_RoccoR80XMC
#include "TRandom3.h"
#include "TMath.h"
...
struct CrystalBall80X{
    ...
    CrystalBall80X(double m_, double s_, double a_, double n_){
	init(m_, s_, a_, n_);
    }
...
const double CrystalBall80X::pi    = TMath::Pi();
const double CrystalBall80X::SPiO2 = sqrt(TMath::Pi()/2.0);
const double CrystalBall80X::S2    = sqrt(2.0);
...
class RocRes80XMC{
	...
	CrystalBall80X  cb[NMAXETA][NMAXTRK]; 
...
class RoccoR80XMC{
	...
	RoccoR80XMC(); //temporary, will change 
	RoccoR80XMC(std::string dirname); //temporary, will change 
	~RoccoR80XMC();
