...
def addMCAnalysisOptions(parser,addTreeToYieldOnesToo=True):
    ...
    parser.add_option("--max-genWeight-procs", dest="maxGenWeightProc", type="string", nargs=2, action="append", default=[], help="Select an absolute value for the maximum genWeight value to be used for a given MC process (first value is a regular expression for the process, second is the max weight). This option effectively applies a cut on the absolute value of the genWeight, and also modifies the sum of genweights using the histogram 'distrGenWeights' saved with the ntuples (so it might not work on older ntuples). Check the histogram content in case the weigth was normalized to the most probable one, so to tune the best value to cut on. Note that the x axis of 'distrGenWeights' is in Log10, the conversion is done internally (you have to pass x, not Log10(x)). The option can be specified more than once for different processes (for the moment it might only be needed for the mW pilotZ  sample)");
