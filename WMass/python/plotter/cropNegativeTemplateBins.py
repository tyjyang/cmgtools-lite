...
def cropNegativeContent(h, silent=True):
    ...
    if not silent:
        print "{n}: integral = {i} changed by {r}".format(n=h.GetName(),
                                                          i=str(integral),
...
if __name__ == "__main__":
    ...
    outfile = outdir + basenameFile.replace(".root","_CROPNEGATIVEBIN.root")
    print "Now copying shapes into a file created in output folder before overwriting some keys"
    print "The copy will be updated"
    cpcmd = "cp {f} {fnew}".format(f=options.infile,fnew=outfile)
    print cpcmd
    os.system(cpcmd)

    tfno = ROOT.TFile(options.infile,'READ')
