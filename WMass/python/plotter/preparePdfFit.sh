...
for charge in plus minus; do
    python makeHistogramsWMass.py -i plots/testNanoAOD/WmassPlots/W${pdf}_testTHn/fakeRateRegion_systTHn/postVFP/allTHn/postprocessing/wmass_shapes.root --outdir cards/wmass/W${pdf}_testTHn/ -c $charge --decorrelate-by-charge ".*effStatTnP|.*muR\d+|.*muF\d+|.*CMS_scale_m.*"
done
