...
# cards/diffXsec_mu_2019_09_19_nativeMCatNLOxsec_1sigBin_4fixedPOI/fit/hessian/fitresults_123456789_Asimov_nativeMCatNLOxsecW_RochesterCorrUncert_cropNegBinNomi_clipSyst1p3_clipSigSyst1p15_clipPtScale1p15_decorrPtScaleSystByEta_FSRshapeOnly_addInclXsec_addImpactsOnMw_fixedPOI_bbb1_cxs1.root
...
if __name__ == "__main__":
    ...
    #parser.add_argument(     '--nuis',       dest='nuis',       default='',   type=str, help='single nuisances for which you want to show the impacts versus pt-eta (can pass comma-separated list to make all of them one after the other)')
    ...
    #parser.add_argument(     '--abs-value', dest='absValue' , default=False , action='store_true',   help='Use absolute values for impacts (groups are already positive)')
    ...
    parser.add_argument(     '--suffix',     dest='suffix',     default='',   type=str, help='suffix for the output name')
    ...
    parser.add_argument(     '--prefitUncertainty'  , dest='prefitUncertainty',      default=50.0, type=float, help='prefit uncertainty on mW in MeV')
    parser.add_argument('-c', '--channel',     dest='channel',     default='mu',  type=str, choices=["mu", "el", "l"], help='Specify channel to make legend. It is no longer guessed from the name of POIs. "l" is for generic lepton in legend, instead of (#mu|e)')
    ...
    if args.channel == "el"   : flavour = "e"
    elif args.channel == "mu" : flavour = "#mu"
    else                      : flavour = "l"
    ...
    for ext in ["pdf", "png"]:
        c1.SaveAs(args.outdir + "/impactsOnMw{sf}_chart.{ext}".format(sf=suffix,ext=ext))
