...
if __name__ == "__main__":
    ...
    parser.add_argument(     '--parNameCanvas',  default='', type=str, help='The canvas name is built using the parameters selected with --params. If they are many, better to pass a name, like QCDscales or PDF for example')
    ...
    parser.add_argument('--which-matrix',  dest='whichMatrix', choices=["both","covariance","correlation"], default='both', type=str, help='Which matrix: covariance|correlation|both')
    ...
    for im,tmp_mat in enumerate(matricesToPlot):
        ...
        if args.outdir:
            ...
            if not args.skipLatexOnTop:
                ...
                lat.DrawLatex(rightTextOffset+offsetLatex, 0.95, '36.3 fb^{-1} (13 TeV)')

            if args.verticalLabelsX: tmp_mat.LabelsOption("v","X")
            if nbins >= 20: tmp_mat.LabelsOption("v","X")

            if args.parNameCanvas: 
                paramsName = args.parNameCanvas
            else : 
                paramsName = args.params.replace(',','AND')
                for x in ['.', '*', '$', '^', '|', '[', ']', '(', ')']:
                    paramsName = paramsName.replace(x,'')

            suff = '' if not args.suffix else '_'+args.suffix
            outfname = args.outdir+'/small{corcov}{suff}_{pn}'.format(suff=suff,pn=paramsName,corcov=corcov)
            for i in ['pdf', 'png']:
