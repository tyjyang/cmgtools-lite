...
def makeSignalModel(model, w):
    if model == "Z-Voit":
        w.factory("Voigtian::signal(mass,sum::(dm[0,-20,10],MZ[91.1876]), GammaZ[2.495],sigma)")
        return (w.pdf("signal"), ["dm", "sigma"])
    if model == "Z-CB":
        w.factory("BreitWigner::zBW(mass,MZ[91.1876], GammaZ[2.495])")
        w.factory("CBShape::resol(mass,dm[0,-30,5], sigma, alpha[3.], n[1])")
...
def makeSumModel(spdf,bpdf,w):
    ...
    w.factory("SUM::sumModel(f_signal[0.5,0.,1]*%s, %s)" % (spdf.GetName(), bpdf.GetName()))
    return (w.pdf("sumModel"), ['f_signal'])
...
def fit1D(hist, options, modelOverride=False):
    ...
    pdf, sumparams = makeSumModel(spdf, bpdf, w)
    result = pdf.fitTo(data, ROOT.RooFit.Save(True))
    return (w,data,pdf,sparams+bparams+sumparams,result)
...
def makeHist1D(tree, options, weightedMC = False):
    ...
    if weightedMC: finalCut = "(%s) * puw2016_nTrueInt_36fb(nTrueInt)*trgSF_We(LepGood1_pdgId,LepGood1_pt,LepGood1_eta,2)*leptonSF_We(LepGood1_pdgId,LepGood1_pt,LepGood1_eta)*leptonSF_We(LepGood2_pdgId,LepGood2_pt,LepGood2_eta)" % finalCut
...
def makeHistsMPtEta(tree, ptedges, etaedges, options,  weightedMC = False):
    hl1 = makeHist3D_(tree, ("probe_pt", ptedges), ("probe_eta", etaedges), options, weightedMC=weightedMC)
...
def addZFitterOptions(parser):
    ...
    parser.add_option("-t", "--tree",    dest="tree", default='L1EG/fitter_tree', help="Tree name");
    parser.add_option("-c", "--cut",     dest="cut", type="string", default="L1EG_bx==-1", help="cut")
    parser.add_option("--xcut",     dest="xcut", type="float", nargs=2, default=None, help="x axis cut")
    parser.add_option("-x", "--x-var",   dest="xvar", type="string", default=("pair_mass","22,35,145"), nargs=2, help="X var and bin")
...
if __name__ == "__main__":
    ...
    if options.mode == "1D":
        ...
        # strip clearly bad fits
        fits = [ (i,res,refres) for (i,res,refres) in fits if abs(res["dm"][0]) < 30 ]
        if options.refmc:
            fits = [ (i,res,refres) for (i,res,refres) in fits if abs(refres["dm"][0]) < 30 ]
        ...
        outFile = ROOT.TFile("{pdir}/{name}.root".format(pdir=options.printDir,name=options.name),'recreate')
        # make 2Ds
        ...
        for i,(bin, res, refres) in enumerate(fits):
           ...
           for x in myparams:
                if x=='f_signal': 
                    hdata[x].SetBinContent(bin[1],bin[0], res[x][0]*hists[binIndex[bin]].Integral())
                    hdata[x].SetBinError(  bin[1],bin[0], res[x][1]*hists[binIndex[bin]].Integral())
                else:
           ...
           if options.refmc:
               for x in myparams:
                   if x=='f_signal': 
                       href[x].SetBinContent(bin[1],bin[0], res[x][0]*hists[binIndex[bin]].Integral())
                       href[x].SetBinError(  bin[1],bin[0], res[x][1]*hists[binIndex[bin]].Integral())
                   else:
              ...
              for x in myparams:
                  if x=='f_signal':
                      text[x].append("%s    %+5.3f +- %5.3f " % (header, res[x][0]*hists[binIndex[bin]].Integral(), res[x][1]*hists[binIndex[bin]].Integral()))
                  else:
