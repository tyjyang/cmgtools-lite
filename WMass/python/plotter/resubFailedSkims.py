...
if __name__ == "__main__":
    ...
    if "_resub" in outdir:
        ...
        outdir.replace("resub"+outdirNoNum, "resub"+string(int(outdirNoNum)+1))
        outdir += "/"
    else:
        outdir += "_resub1/"
    ...
    input_samples = [f for f in os.listdir(options.ntuplesDirIn) if not f.endswith(".txt")]
    output_samples = [f for f in os.listdir(options.ntuplesDirOut) if not f.endswith(".txt")]
    missing_samples = []
    ...
    if len(missing_samples):
        print"="*30
        print "There are %d missing samples:" % len(missing_samples)    
        print"-"*30
    ...
    for thisf in condor_files:
        ...
        f  = open(thisf,"r")
        for line in f:
            if any(line.startswith(x) for x in ["arguments", "queue", "  ", "\n"]):
